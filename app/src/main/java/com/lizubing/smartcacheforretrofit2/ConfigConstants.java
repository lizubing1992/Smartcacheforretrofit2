...
public class ConfigConstants {
  ...
  public static final int MAX_SMALL_DISK_VERYLOW_CACHE_SIZE = 100 * ByteConstants.MB;//小图极低磁盘空间缓存的最大值（特性：可将大量的小图放到额外放在另一个磁盘空间防止大图占用磁盘空间而删除了大量的小图）
  public static final int MAX_SMALL_DISK_LOW_CACHE_SIZE = 100 * ByteConstants.MB;//小图低磁盘空间缓存的最大值（特性：可将大量的小图放到额外放在另一个磁盘空间防止大图占用磁盘空间而删除了大量的小图）
  public static final int MAX_SMALL_DISK_CACHE_SIZE = 200 * ByteConstants.MB;//小图磁盘缓存的最大值（特性：可将大量的小图放到额外放在另一个磁盘空间防止大图占用磁盘空间而删除了大量的小图）
   
  public static final int MAX_DISK_CACHE_VERYLOW_SIZE = 200 * ByteConstants.MB;//默认图极低磁盘空间缓存的最大值
  public static final int MAX_DISK_CACHE_LOW_SIZE = 500 * ByteConstants.MB;//默认图低磁盘空间缓存的最大值
  public static final int MAX_DISK_CACHE_SIZE = 1500 * ByteConstants.MB;//默认图磁盘缓存的最大值
   
   
  private static final String IMAGE_PIPELINE_SMALL_CACHE_DIR = "18touch_mengju/imagepipeline_cache";//小图所放路径的文件夹名
  private static final String IMAGE_PIPELINE_CACHE_DIR = "18touch_mengju/imagepipeline_cache";//默认图所放路径的文件夹名
...
//      sImagePipelineConfig = configureCaches(context);
  ...
  private static ImagePipelineConfig configureCaches(Context context) {
    ...
    //小图片的磁盘配置
      DiskCacheConfig diskSmallCacheConfig =  DiskCacheConfig.newBuilder(MyApplication.getContext())
...
//            .setVersion(version)
    ...
    //默认图片的磁盘配置
      DiskCacheConfig diskCacheConfig =  DiskCacheConfig.newBuilder(MyApplication.getContext())
...
//    return configBuilder.build();
      ...
      ImagePipelineConfig config = MyOkHttpImagePipelineConfigFactory
...
public static void init(final Resources resources) {
        if (sPlaceholderDrawable == null) {
          sPlaceholderDrawable = resources.getDrawable(R.mipmap.ic_launcher);
        }
        if (sErrorDrawable == null) {
          sErrorDrawable = resources.getDrawable(R.mipmap.ic_launcher);
        }
      }
